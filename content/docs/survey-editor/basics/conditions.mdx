...
* **How it works:** You select the item key and validation key of a previous question that has validation rules applied to it. The condition evaluates to `true` if the participants input for that question passes all its defined validation rules.
